...
_Last updated: 2026-06-04 — Phase 2 (Coordinate Mapping & Fixture Viewer) complete: the API→Three.js coordinate mapping is golden-locked against a real captured fixture (incl. a rotated box), and the `/result` route renders it as a faithful, explorable 3D pallet — colour-by-type legend, orbit/zoom/pan, and animated ISO/TOP/FRONT camera presets — with three/r3f/drei isolated to the lazy `/result` chunk._
