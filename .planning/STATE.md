...
| 260619-shd | Fast: cap viewer max shade to ~30% of current per user — flatten the shading WITHOUT over-brightening (the prior fog/brightness bump broke it, so fog + ambient left untouched). Shade came from the hemisphere top/bottom gradient + key falloff: hemisphereLight groundColor #737d9c→#c2cae0 (near sky #cdd7f0, kills the gradient so undersides aren't darkened), key directionalLight 0.23→0.08, secondary fill 0.15→0.06; ambient 0.8 + fog 2800/5600 unchanged. Darkest face now within ~10% of brightest (≈30% of the prior ~31% shade), overall brightness ~same. | 2026-06-19 | 48e0b62 | Verified (gates green: typecheck, lint, 236 unit, 9 e2e) | — |
