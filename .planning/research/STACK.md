...
**Domain:** Self-hostable, stateless React SPA — form-heavy config UI + async API polling + react-three-fiber 3D viewer, served as a static Docker image
**Researched:** 2026-06-03
**Confidence:** HIGH (all versions verified live against the npm registry on 2026-06-03; peer-dependency constraints read directly from published package manifests)
...
**The one footgun that drives everything below:** the current `@react-three/fiber@9` line requires **React 19** (`react >=19 <19.3`), and `@react-three/drei@10` requires `react ^19` + `@react-three/fiber ^9`. You cannot mix React 18 with current r3f/drei majors. Pick React 19 and stay there. Full matrix in [Version Compatibility](#version-compatibility).
...
> **Note on drei vs raw three for camera presets:** the mockup animates camera moves manually (`lerpVectors`). With drei you can use `<CameraControls>` (`.setLookAt(..., true)` for animated transitions) or `<Bounds>` to fit. Prefer drei's controls over re-porting the imperative animation.
...
**Why Tailwind, given the mockups are hand-written CSS with custom properties?**

The mockups are a *design spec*, not the implementation, and they are already built on a CSS-variable design-token system (`--accent:#4f46e5`, `--radius`, `--shadow`, the Inter/JetBrains-Mono pair). Tailwind v4 maps onto that exactly: drop the existing `:root` tokens into an `@theme` block and they become utilities (`bg-accent`, `rounded-sm`, `font-mono`). You reproduce the look faithfully while gaining co-located styling that survives heavy component refactors — which a dynamic catalog UI will undergo.
...
npm install -D vite@8 @vitejs/plugin-react@6 typescript@~6.0
...
npm install @tanstack/react-query@5 react-hook-form@7 zod@4 @hookform/resolvers@5

# Routing + small utils
npm install react-router@7 nanoid@5 clsx@2

# Styling (Tailwind v4 — CSS-first, Vite plugin)
npm install -D tailwindcss@4 @tailwindcss/vite@4

# Testing
npm install -D vitest@4 jsdom @testing-library/react@16 @testing-library/jest-dom@6 @testing-library/user-event@14
npm install -D @playwright/test@1

# Lint / format
npm install -D eslint@10 typescript-eslint@8 eslint-plugin-react-hooks eslint-plugin-react-refresh prettier@3 eslint-config-prettier@10
npm install -D vite-tsconfig-paths@6   # optional
```
...
The single most important integration shape. Sketch (not final code):

```ts
// submit
const submit = useMutation({
  mutationFn: (req: PackRequest) => api.post('/api/v1/pack', req), // -> 202 { job_id, status }
});

// poll, started once we have a job_id
const job = useQuery({
  queryKey: ['job', jobId],
  queryFn: () => api.get(`/api/v1/jobs/${jobId}`).then(r => jobSchema.parse(r)), // zod-validate
  enabled: !!jobId,
  refetchInterval: (q) => {
    const s = q.state.data?.status;
    return s === 'done' || s === 'failed' || s === 'timeout' ? false : 1500; // stop polling
  },
});
```

Key points: stop the interval by returning `false`; validate every poll response with the zod `jobSchema` so a contract drift becomes a clean error state, not a crash; map `failed`/`timeout`/unpacked-items onto the graceful-messaging requirement.
...
**If a contributor cannot use React 19 (must stay React 18):**
...
**If bundle size becomes a hard constraint:**
...
**If offline/air-gapped self-hosting matters:**
...
**The critical matrix — verified from published package manifests on 2026-06-03:**
...
**Legacy / React-18 fallback row (use only under constraint):**
...
Multi-stage: Node builds the static bundle, nginx serves it. Optimized for a tiny final image.
...
server {
  ...
  # SPA fallback — every unknown path serves index.html so react-router handles it
  location / {
    try_files $uri $uri/ /index.html;
  }

  # cache hashed assets aggressively, never cache the HTML shell
...
---
*Stack research for: self-hostable stateless React + r3f SPA over an async packing API*
*Researched: 2026-06-03*
